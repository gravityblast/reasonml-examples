...
let drawPlayer (ctx : Canvas.context) ({position: (x, y), size: size} : state) => {
...
let update dt (state : state) : state => {
  ...
  let newX = x +. (state.speed *. dx *. dt /. 1000.);
  let newY = y +. (state.speed *. dy *. dt /. 1000.);

  let newDx = if (newX +. size > 500. || newX < 0.) (dx *. -1.) else dx;
  let newDy = if (newY +. size > 500. || newY < 0.) (dy *. -1.) else dy;
