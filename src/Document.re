...
external _requestAnimationFrame : Dom.window => (unit => unit) => unit = "requestAnimationFrame" [@@bs.send];

let requestAnimationFrame (f : unit => unit) =>
  _requestAnimationFrame window f;
